...
const { ALGORITHM_VERSION, collectScoringSignals, computeScoreV2 } = require("../../../packages/geo-core/scoring/scoring-v2.js");
...
const { SCORING_VERSION, computeGeoAssessment } = require("../../../packages/geo-core/scoring/geo-assessment.js");
const { AI_TRUST_INDEX_VERSION, computeAiTrustIndex } = require("../../../packages/geo-core/scoring/ai-trust-index.js");
const { PARSER_VERSION } = require("../../../packages/geo-core/evidence/brand-match.js");
const { resolveCitationRedirects } = require("../../../packages/crawler/citation-resolve.js");
const {
  QUERY_PLANNER_VERSION,
  buildGeoQueryPlanResolved,
  normalizeReviewedQueryPlan
} = require("./query-planner.js");
...
async function resolveQueryPlanning(input, options = {}) {
  ...
  const preferredQueries = Array.isArray(options.customQueries)
    ? options.customQueries.map((query) => String(query || "").trim()).filter(Boolean).slice(0, 4)
    : [];
  try {
    const planner = typeof options.queryPlanner === "function" ? options.queryPlanner : buildGeoQueryPlanResolved;
    return await planner(input, { ...options.queryPlannerOptions, preferredQueries });
  } catch (error) {
    return {
      status: "unavailable",
