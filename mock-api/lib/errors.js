...
function hintForError(error) {
  if (error.stage === "config") return "Check DEEPSEEK_API_KEY and PERPLEXITY_API_KEY in .env, then restart the server.";
  ...
  if (error.stage === "deepseek_api") return "DeepSeek may be rate limited, overloaded, or rejecting the request. Check DEEPSEEK_MODEL and DEEPSEEK_API_KEY.";
  if (error.stage === "deepseek_json") return "DeepSeek returned text that could not be parsed as JSON. Retry with the structured-output setting.";
