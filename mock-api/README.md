...
專用資料庫是 `geocheck-reports`，migration 位於 `mock-api/migrations/`，專案內的 `mock-api/wrangler.jsonc` 僅用來管理它，**不代表 Render Node 服務已遷移到 Cloudflare Worker**。要啟用 Render 的持久保存，建立只含此資料庫 **D1 Read + D1 Write** 權限的 API token，並設定：
