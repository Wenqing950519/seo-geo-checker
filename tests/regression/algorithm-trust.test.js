...
const { applyV2Audit } = require("../../apps/web/report/real-lite-audit-v2-core.js");
...
assert.equal(unavailableResult.score.evidence_status, "unknown");
...
assert.equal(resultA.score.value, 100, "AI Trust is derived from answer and URL evidence, not the model-provided score field");
assert.ok(Number.isFinite(resultA.score.value));
assert.equal(resultA.score.algorithm_version, "ai-trust-1.0.0");
assert.equal(resultA.score.evidence_status, "measured");
assert.equal(resultA.score.value, 100, "fully adopted answers with official URL evidence produce 100 AI Trust");
