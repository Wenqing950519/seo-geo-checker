...
if (!registerPath || !outputPath) throw new Error("Usage: node filter_supporting_store_leads.mjs <register.csv> <output.csv>");

const acceptedTypes = new Set(["official_venue_page", "booking_platform"]);
const rows = readCsv(registerPath).filter((row) => acceptedTypes.has(row.source_type));
