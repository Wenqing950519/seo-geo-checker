...
## D-053 — Platform 綠界個人申請、預付額度優先（2026-09-20）

- **來源與狀態**：使用者本次明確指定「最基礎的個人申請方案」，並表示先付儲值式若能跑通就優先採用；Confirmed（方向，非承作或正式收款已核准）。
- **決策**：採綠界一般賣家、個人身分準備申請；新方案預付服務額度、按用量扣款，不沿用訂閱。後付僅在預付的法規或承作條件無法滿足時再評估，不以先前未確認的禮券推論直接排除先付。
- **委託範圍**：使用者授權依官方要求擬定一般退款與保存政策。草案採未用實付餘額可退、不扣退款費、不設餘額效期、停止服務返還；個資依目的必要期間、法定義務與刪除權利處理。這些是委託起草的政策，並非已有官方通用 API 範本或已驗證清理／退款功能。
