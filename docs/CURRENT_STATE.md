...
`[公開頁驗證 2026-09-10]` 正式 `https://geocheck.lisheng.cv/privacy`、`/terms` 與 `/refund` 均回 HTTP 404；GitHub `main` 已含對應靜態檔與 `services/api/server.js` 路由，故正式 Render 服務尚未同步到該版本。這會阻斷目前已填入 Google branding 的隱私與條款網址驗證；在 Render 發佈並以三個 URL 回 200 前，不得送交 OAuth scope verification。
