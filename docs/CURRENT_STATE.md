...
`[公開頁驗證 2026-09-10]` 正式 `https://geocheck.lisheng.cv/privacy`、`/terms` 與 `/refund` 均回 HTTP 404；GitHub `main` 已含對應靜態檔與 `services/api/server.js` 路由，且同步 commit `062693b` 已推送至 origin，但正式 Render 服務未自動發佈該 commit。這會阻斷目前已填入 Google branding 的隱私與條款網址驗證；在 Render 發佈並以三個 URL 回 200 前，不得送交 OAuth scope verification。
...
`[A Dashboard 同步 2026-09-10]` A browser app 的 fixture mode 已改為預設關閉；網路或 API 失敗會顯示失敗，不再靜默改以 fixture 偽裝為真實 Dashboard。已登入時，Project、entitlement、overview、performance、questions、citations 與 data-quality 皆從 `/app-api/v1` 讀取；新增 Project 走 server API，單題本機模擬與 evidence fixture fallback 已移除。此變更尚未隨 Render 主站發佈，因此不是正式站完成證據。
