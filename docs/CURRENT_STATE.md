...
## Platform 金流準備（2026-09-20，本機未部署）

已實查綠界申請、費率與串接規格，並以瀏覽器確認 Platform 正式站的 `/pricing`、`/terms` 仍顯示舊 Product A 月費與藍新訂閱內容。本機已新增 Platform 專用計費／條款／隱私／退款草案、修正資產優先路由與入口，相關測試及 Worker dry-run 通過；正式站尚未更新。沒有送件、付款交易、退款或金流開通證據。D-053 已確認個人一般賣家與預付優先；退款與保存政策依使用者委託補齊草案。尚缺單價及交付條件、個人預付承作及履約保障答覆、KYC，以及帳本／退款／清理的實作驗證，不能把配額或上游成本紀錄當客戶帳單。詳見 [綠界送審準備](developer-api/ECPAY_READINESS.md)。
