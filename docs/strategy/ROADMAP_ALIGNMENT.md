...
資料夾應同時支援 A、未來 Developer API，以及企劃書的 Project／Monitoring 工作流；不應把「所有能力對外 API 化」設為建立第二次使用的必經之路。
...
該提案與 `MULTI_ENGINE_API_SERVICE_BRIEF.md` 的重心不同：前者先驗證 practitioner 重複使用，後者偏向開發者服務與多引擎。使用者本次確認的是資料夾整理並考慮未來路線，沒有正式選擇新的收費單位或商業優先序。這兩條路線可共用 Run／evidence 基礎，但不能偷偷將其中一份草案升格為正式決策。
...
| 下一個產品驗證候選 | Account／Project、保存 Run、最小 Dashboard、第二次 review | services/api 的 domain/storage + apps/web 的登入工作區；不需要先發布 SDK 或另一個 console |
| workflow 成立後 | 有限排程、跨期比較、用量與成本可見 | packages/monitor 消費服務 use case；每次 run 有 immutable evidence、失敗與版本 |
| 外部整合需求成立後 | public API + SDK + developer console | services/api/v1 與 packages/sdk；console 只管理 key／用量／整合，不重做商家 Dashboard |
| 更後期的假設 | 第二引擎、GSC／intent、agency report、Agent 輔助解讀 | 需求與測量有效性驗證後另定邊界；不建立空的企業功能目錄 |
