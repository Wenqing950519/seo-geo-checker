This file contains mandatory project instructions for Claude Code.
Use /memory when needed to verify which project and user memories are loaded.
Do not rely on auto memory as the sole source of project truth.
...
以下規則檔為本 Contract 的細則，**開始對應類型的任務前必須讀取**（Claude Code 不會自動載入這些檔案）：

| 檔案 | 何時必讀 |
|---|---|
| `.claude/rules/research.md` | 研究設計、資料解讀、白皮書、對外結論 |
| `.claude/rules/learning.md` | 任何涉及使用者理解與決策的任務 |
| `.claude/rules/coding.md` | 修改程式、測試、架構變更 |
